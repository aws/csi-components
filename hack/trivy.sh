...
if [ -n "${OUTPUT_SARIF:+x}" ]; then
  docker run --rm -v /var/run/docker.sock:/var/run/docker.sock:ro public.ecr.aws/aquasecurity/trivy:0.69.2 image -f sarif "${REGISTRY}/${IMAGE}:${TAG}-eksbuild.${EKSBUILD}" > "${BASE_DIR}/../output/${IMAGE}.sarif"
  ...
  docker run --rm -v /var/run/docker.sock:/var/run/docker.sock:ro public.ecr.aws/aquasecurity/trivy:0.69.2 image -q "${REGISTRY}/${IMAGE}:${TAG}-eksbuild.${EKSBUILD}"
fi
